...
{//直接拷贝给每个网络发送对象 
	...
	if (enable_mp4 == true && recordMP4 == 0)
	{
		...
		if (recordFileSource)
		{
			...
			WriteLog(Log_Debug, "重新设置 app = %s ,stream = %s 录像保存时长为 %llu 小时 ", app, stream, fileKeepMaxTime);
		}
...
void   CMediaStreamSource::SetG711ConvertAAC(int nFlag)
{
	...
	WriteLog(Log_Debug, "CMediaStreamSource = %X, app = %s ,stream = %s , SetG711ConvertAAC() 设置音频是否转码参数 nG711ConvertAAC = %d ", this, app, stream, nG711ConvertAAC);
}
