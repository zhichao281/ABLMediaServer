...
private:
  uint32_t fState[4]; // ABCD
  uint64_t fBitCount; // number of bits, modulo 2^64
...
void MD5Context::addData(unsigned char const* inputData, unsigned inputDataSize) {
  ...
  uint64_t const byteCount = fBitCount>>3;
...
static void unpack32(unsigned char out[4], uint32_t in) {
  for (unsigned i = 0; i < 4; ++i) {
    out[i] = (unsigned char)((in>>(8*i))&0xFF);
  }
}

static void unpack64(unsigned char out[8], uint64_t in) {
...
void MD5Context::transform64Bytes(unsigned char const block[64]) {
  uint32_t a = fState[0], b = fState[1], c = fState[2], d = fState[3];
