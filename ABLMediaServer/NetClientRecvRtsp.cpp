...
extern CMediaFifo                            pReConnectStreamProxyFifo;  //需要重新连接代理ID 
			...
			if (pRtsp->cbVideoTimestamp != 0 && pRtsp->cbVideoTimestamp != timestamp)
			{
				...
				{//如果是宇视摄像头需要判断视频编码格式
					if (pRtsp->CheckVideoIsIFrame("H264", pRtsp->szCallBackVideo, pRtsp->cbVideoLength))
					{
						...
						WriteLog(Log_Debug, "nClient = %llu ，宇视的视频源，已经判断出视频为 H264 ", pRtsp->nClient);
					}
					else if (pRtsp->CheckVideoIsIFrame("H265", pRtsp->szCallBackVideo, pRtsp->cbVideoLength))
					{
						...
						pRtsp->bUniviewFlag = false;
						WriteLog(Log_Debug, "nClient = %llu ，宇视的视频源，已经判断出视频为 H265 ", pRtsp->nClient);
					}
					else
					{
						if (GetTickCount64() - pRtsp->nCreateDateTime > 1000 * 10)
						{//长时间判断视频编码格式失败，需要切换另外一种编码器名称的rtp解包
							rtp_payload_decode_destroy(pRtsp->rtpDecoder[0]);
							if (strcmp(pRtsp->szVideoName, "H264") == 0)
 							   strcpy(pRtsp->szVideoName, "H265");
 							else 
	...
	{//剩余空间不够，需要把剩余的buffer往前移动
		...
		{//如果有少量剩余
			...
			if (MaxNetDataCacheCount - nNetEnd < nDataLength)
			{
				...
				WriteLog(Log_Debug, "CNetClientRecvRtsp = %X nClient = %llu 数据异常 , 执行复位 ", this, nClient);
				//destroy();
				return 0;
			}
		}
...
void  CNetClientRecvRtsp::InputRtspData(unsigned char* pRecvData, int nDataLength)
{
	...
	{//&& nRecvLength > 500
		...
		{//rtsp重定向
			...
			{//rtsp 有错
				WriteLog(Log_Debug, "rtsp 重定向url有错 %s ", szNewRtspURL);
 				pDisconnectBaseNetFifo.push((unsigned char*)&nClient, sizeof(nClient));
				return;
			}
			else
			{//rtsp正确
				...
				if (pParentPtr != NULL)
				{
					...
					WriteLog(Log_Debug, "rtsp 重定向 %s", pParentPtr->m_addStreamProxyStruct.url);
...
bool   CNetClientRecvRtsp::GetMediaInfoFromRtspSDP()
{
	...
	{
		WriteLog(Log_Debug, "nClient = %llu ,在SDP中没有找到视频、音频信息 \r\n%s", nClient, szRtspContentSDP);
		return false;
 	}
	...
	if (sipParseV.GetSize() > 0)
	{
		...
		else if (sipParseV.GetFieldValue("m=video", szTemp))
		{
			...
			bUniviewFlag = true; //确定为宇视的流媒体，需要判断视频编码格式
...
int CNetClientRecvRtsp::ProcessNetData()
{
	...
	while (!bIsInvalidConnectFlag && bRunFlag && netDataCacheLength > 4)
	{
		...
		{
			WriteLog(Log_Debug, "CNetClientRecvRtsp= %X  , ProcessNetData() ,RTSP 网络处理超时 ! , nClient = %llu ， netDataCacheLength = %d ", this, nClient, netDataCacheLength);
			break;
		}
	}
