...
class PeerConnectionManager 
{
	class VideoSink : public rtc::VideoSinkInterface<webrtc::VideoFrame> {
		public:
			VideoSink(const rtc::scoped_refptr<webrtc::VideoTrackInterface> & track): m_track(track) {
				RTC_LOG(LS_INFO) << __FUNCTION__ << " videotrack:" << m_track->id();
			...
			virtual void OnFrame(const webrtc::VideoFrame& video_frame) {
				...
				RTC_LOG(LS_VERBOSE) << __FUNCTION__ << " frame:" << buffer->width() << "x" << buffer->height();
			}
	...
	class AudioSink : public webrtc::AudioTrackSinkInterface {
		public:
			AudioSink(const rtc::scoped_refptr<webrtc::AudioTrackInterface> & track): m_track(track) {
				RTC_LOG(LS_INFO) << __FUNCTION__ << " audiotrack:" << m_track->id();
	...
	class CreateSessionDescriptionObserver : public webrtc::CreateSessionDescriptionObserver {
		public:
			...
			virtual void OnSuccess(webrtc::SessionDescriptionInterface* desc)
			{
				std::string sdp;
				desc->ToString(&sdp);
				RTC_LOG(LS_INFO) << __FUNCTION__ << " type:" << desc->type() << " sdp:" << sdp;
				if (!m_cancelled) {
	...
	class DataChannelObserver : public webrtc::DataChannelObserver  {
		public:
			...
			virtual void OnStateChange() {
				RTC_LOG(LS_ERROR) << __FUNCTION__ << " channel:" << m_dataChannel->label() << " state:"<< webrtc::DataChannelInterface::DataStateString(m_dataChannel->state());
			...
			virtual void OnMessage(const webrtc::DataBuffer& buffer) {
				...
				RTC_LOG(LS_ERROR) << __FUNCTION__ << " channel:" << m_dataChannel->label() << " msg:" << msg;
			}
	...
	class PeerConnectionObserver : public webrtc::PeerConnectionObserver {
		public:
			...
			virtual void OnAddStream(rtc::scoped_refptr<webrtc::MediaStreamInterface> stream)    {
				RTC_LOG(LS_ERROR) << __FUNCTION__ << " nb video tracks:" << stream->GetVideoTracks().size();
				webrtc::VideoTrackVector videoTracks = stream->GetVideoTracks();
				if (videoTracks.size()>0) {					
					m_videosink.reset(new VideoSink(videoTracks.at(0)));
				}
				RTC_LOG(LS_ERROR) << __FUNCTION__ << " nb audio tracks:" << stream->GetAudioTracks().size();
			...
			virtual void OnRenegotiationNeeded()                              {
				RTC_LOG(LS_ERROR) << __FUNCTION__ << " peerid:" << m_peerid;;
			}
	...
	public:
		...
		void addIceServers(const std::string turnurl);
