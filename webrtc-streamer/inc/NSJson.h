...
public:
	...
	// 解析Json格式字符串
	static NSJsonObject ParseStr(const std::string& strJson);

	// 读取文件，解析json
	static NSJsonObject ParaseFile(const std::string& strJson);

	// 保存到文件
	static bool SaveToFile(const std::string& strJson, NSJsonObject JsonObj);
};
